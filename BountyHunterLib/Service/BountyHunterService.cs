...
namespace BountyHunterLib.Service
{
    public class BountyHunterService : IBountyHunterService
    {
        ...
        public HunterListModel GetAllHunters()
        {
            ...
            using (var db = new BountyHuntersEntities1())
            {
                //db.TheHunters.Add()

                var query = db.TheHunters.Select(x => x);

                var hunterList = query.ToList();
                ...
                hunterList.ForEach(hunter =>
                {
                    model.Hunters.Add(
                        new HunterModel()
                        {
                            ...
                            Wage = hunter.Wage,
                            DetailOne = hunter.DetailOne,
                            DetailTwo = hunter.DetailTwo,
                            DetailThree = hunter.DetailThree
                        }
        ...
        //public HunterModel Add(HunterModel h)
        //{
        ...
        public void AddNewHunter(HunterModel hm)
        {
            using (var db = new BountyHuntersEntities1())
            {
                ...
                //model.Hunters.Add(hm);

                //hm = new TheHunter();
                db.TheHunters.Add(new TheHunter()
                {

                    //ID = db.TheHunters.Count() + 1,
                    Name = hm.Name,
                    ImageUrl = hm.ImageUrl,
                    Wage = hm.Wage,
                    Description = hm.Description,
                    DetailOne = hm.DetailOne,
                    DetailTwo = hm.DetailTwo,
                    DetailThree = hm.DetailThree
                });

                //model.Hunters.Add(newHunter);

                //hunterList.Add();
                //db.TheHunters.Add(newHunter);
                db.SaveChanges();
        ...
        public void DeleteHunterById(int id)
        {
            using (var db = new BountyHuntersEntities1())
            {
                var hunterToDelete = db.TheHunters.Where(x => x.ID == id).First();

                db.TheHunters.Remove(hunterToDelete);
                db.SaveChanges();
            }
        }

        public void UpdateHunter(HunterModel hunterModel)
        {
            //var hm = GetHunterById(id);
            using (var db = new BountyHuntersEntities1())
            {
                //var hunterModel = GetHunterById(id);
               
                var existingHero = db.TheHunters.Where(h => h.ID == hunterModel.HunterId).First();

                
                existingHero.Name = hunterModel.Name;
                existingHero.ImageUrl = hunterModel.ImageUrl;
                existingHero.Wage = hunterModel.Wage;
                existingHero.Description = hunterModel.Description;
                existingHero.DetailOne = hunterModel.DetailOne;
                existingHero.DetailTwo = hunterModel.DetailTwo;
                existingHero.DetailThree = hunterModel.DetailThree;
